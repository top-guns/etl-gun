...
export type Condition<T> = Record<keyof T, any | ((value: any) => (v: T) => boolean)> | ((value: any) => boolean);


export function isMatch<T>(value: T, condition: Condition<T>, rootProperty?: string) {
    ...
    for (const key in condition) {
        ...
        const res: boolean = typeof (condition as any)[key] === 'function' ? (condition as any)[key](val) : (condition as any)[key] == val;
        if (!res) return false;
    }
