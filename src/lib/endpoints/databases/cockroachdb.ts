...
export class Endpoint extends KnexEndpoint {
    ...
    get displayName(): string {
        if (typeof this.config.connection === 'string') return `CockroachDB (${this.config.connection})`;

        const connection = this.config.connection as ConnectionConfig;
        if (typeof connection.host !== 'undefined') return `CockroachDB (${connection.host}:${connection.port}/${connection.database})`;
        
        return `CockroachDB (${this.instanceNo})`;
    }
}
