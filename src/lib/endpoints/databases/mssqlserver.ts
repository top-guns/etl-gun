...
export class Endpoint extends KnexEndpoint {
    ...
    constructor(connection: any, pool?: PoolConfig) {
        super('mssql', connection, pool);
    }

    get displayName(): string {
        if (typeof this.config.connection === 'string') return `MS SQL Server (${this.config.connection})`;

        const connection = this.config.connection as ConnectionConfig;
        if (typeof connection.host !== 'undefined') return `MS SQL Server (${connection.host}:${connection.port}/${connection.database})`;
        
        return `MS SQL Server (${this.instanceNo})`;
    }
}
