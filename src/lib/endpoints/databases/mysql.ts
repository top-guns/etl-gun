...
export class Endpoint extends KnexEndpoint {
    ...
    get displayName(): string {
        if (typeof this.config.connection === 'string') return `MySQL (${this.config.connection})`;

        const connection = this.config.connection as ConnectionConfig;
        if (typeof connection.host !== 'undefined') return `MySQL (${connection.host}:${connection.port}/${connection.database})`;

        return `MySQL (${this.instanceNo})`;
    }
}
