...
export class Endpoint extends KnexEndpoint {
    ...
    get displayName(): string {
        if (typeof this.config.connection === 'string') return `PosgreSQL (${this.config.connection})`;

        const connection = this.config.connection as ConnectionConfig;
        if (typeof connection.host !== 'undefined') return `PosgreSQL (${connection.host}:${connection.port}/${connection.database})`;
        
        return `PosgreSQL (${this.instanceNo})`;
    }
}
