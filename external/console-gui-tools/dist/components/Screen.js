 ...
 */
class Screen extends events_1.EventEmitter {
    ...
    write(...args) {
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (arg.text !== undefined) {
                ...
                const style = { ...arg.style, index: [row.length, row.length + txt.length] };
    ...
    print() {
        this.buffer.forEach((row, i) => {
            ...
            row.styleIndex.forEach(style => {
                const color = style.color ? chalk_1.default[style.color] : (_in) => _in;
                const bg = style.bg ? chalk_1.default[style.bg] : (_in) => _in;
                const italic = style.italic ? chalk_1.default.italic : (_in) => _in;
                const bold = style.bold ? chalk_1.default.bold : (_in) => _in;
                const dim = style.dim ? chalk_1.default.dim : (_in) => _in;
                const underline = style.underline ? chalk_1.default.underline : (_in) => _in;
                const overline = style.overline ? (_in) => _in : (_in) => _in;
                const inverse = style.inverse ? chalk_1.default.inverse : (_in) => _in;
                const hidden = style.hidden ? chalk_1.default.hidden : (_in) => _in;
                const strikethrough = style.strikethrough ? chalk_1.default.strikethrough : (_in) => _in;
